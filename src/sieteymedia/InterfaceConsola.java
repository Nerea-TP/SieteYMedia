...
public class InterfaceConsola {
    ...
    SieteYMedia sietemedia;
    InterfaceConsola(){
        sietemedia=new SieteYMedia();
        presentarJuego();
        turnoJugador();
        turnoBanca();
    }
    ...
    void turnoJugador() {
        ...
        while (!sietemedia.esFinTurno()&& opc == 'C') {
            ...
            Carta[] cartasJugador=sietemedia.getCartasJugador();
            mostrarCartas(cartasJugador);
            double valor = sietemedia.getValorCartasJugador();
    ...
    void turnoBanca() {
        ...
        while (sietemedia.getValorCartasBanca() < valor) {
            sietemedia.darCartasBanca();
        }
        System.out.println("Éstas son mis cartas(banca):");
        Carta[] cartasBanca=sietemedia.getCartasBanca();
        mostrarCartas(cartasBanca);
        double valor1=sietemedia.getValorCartasBanca();
        System.out.println("\nValor de mis cartas(banca): " + valor1);
        if (valor1 > 7.5) {
